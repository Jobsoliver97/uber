...
# NOVO: Adicionar despesa extra manualmente
st.subheader("➕ Adicionar Despesa Extra Manualmente")
with st.expander("Clique aqui para registrar"):
...
if not df.empty:
    meses_disponiveis = df['Data_str'].dropna().apply(lambda x: x[:7]).unique()
    mes = st.selectbox("Escolher mês:", meses_disponiveis)

    if mes:
        corridas_mes = df[df['Data_str'].str.startswith(mes)]
        extras_mes = despesas_extras[
            despesas_extras['Data'].apply(lambda d: d.strftime('%Y-%m')).eq(mes)
        ]
        total_receita = corridas_mes['Receita'].sum()
        total_km = corridas_mes['KM'].sum()
        total_despesa_extras = extras_mes['Valor'].sum()
        saldo = total_receita - total_despesa_extras
        custo_km = total_despesa_extras / total_km if total_km > 0 else 0.0

        st.metric("Receita", f"R$ {total_receita:.2f}")
        st.metric("Despesa (somente extras)", f"R$ {total_despesa_extras:.2f}")
        st.metric("Saldo", f"R$ {saldo:.2f}")
        st.metric("KM Percorridos", f"{total_km:.2f} km")
        st.metric("Custo por KM", f"R$ {custo_km:.2f}/km")
